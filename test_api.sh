...
test_health_checks() {
    ...
    if curl -s -o /dev/null -w "%{http_code}" ${BACKEND_URL}/api/auth/login | grep -q "404\|200"; then
...
test_registration() {
    ...
    if echo "$MENTOR_RESPONSE" | grep -q "\"success\":true"; then
        ...
        print_fail "Mentor registration failed"
    fi
    ...
    if echo "$STUDENT_RESPONSE" | grep -q "\"success\":true"; then
        ...
        print_fail "Student registration failed"
    fi
}
...
test_authentication() {
    ...
    AUTH_RESPONSE=$(curl -s -X GET ${BACKEND_URL}/api/auth/me \
        -H "Authorization: Bearer ${STUDENT_TOKEN}")
    
    if echo "$AUTH_RESPONSE" | grep -q "Test Student"; then
...
test_student_endpoints() {
    ...
    if echo "$PROFILE_RESPONSE" | grep -q "8.5"; then
        print_pass "Student profile retrieved"
    else
        print_fail "Profile retrieval failed"
    fi
}
...
test_rag_endpoints() {
    ...
    if echo "$RAG_QUERY" | grep -q "\"success\":true"; then
        print_pass "RAG query working"
    else
        print_fail "RAG query failed"
    fi
    ...
    if echo "$RAG_CHAT" | grep -q "\"success\":true"; then
        print_pass "RAG chat working"
    else
        print_fail "RAG chat failed"
    fi
    ...
    if echo "$STATS" | grep -q "total_documents"; then
        ...
        print_fail "Stats retrieval failed"
    fi
}
...
test_chat_endpoints() {
    ...
    if echo "$AI_CHAT" | grep -q "\"success\":true"; then
        print_pass "AI chatbot working"
    else
        print_fail "AI chatbot failed"
    fi
    ...
    if echo "$UNREAD" | grep -q "\"success\":true"; then
        print_pass "Unread count endpoint working"
    else
        print_fail "Unread count failed"
    fi
}
...
test_alert_endpoints() {
    ...
    ALERTS=$(curl -s -X GET "${BACKEND_URL}/api/alerts/my-alerts?limit=10" \
        -H "Authorization: Bearer ${STUDENT_TOKEN}")
    
    if echo "$ALERTS" | grep -q "\"success\":true"; then
        print_pass "Alerts retrieved"
    else
        print_fail "Alerts retrieval failed"
    fi
}
...
test_admin_endpoints() {
    ...
    if echo "$STATS" | grep -q "\"users\""; then
        ...
        print_fail "Dashboard stats failed"
    fi
    ...
    if echo "$STUDENTS" | grep -q "\"success\":true"; then
        ...
        print_fail "Students list failed"
    fi
}
...
test_mentor_endpoints() {
    ...
    MENTOR_PROFILE=$(curl -s -X GET ${BACKEND_URL}/api/mentors/me \
        -H "Authorization: Bearer ${MENTOR_TOKEN}")
    
    if echo "$MENTOR_PROFILE" | grep -q "Test Mentor"; then
        print_pass "Mentor profile retrieved"
    else
        print_fail "Mentor profile failed"
    fi
}
...
test_risk_prediction() {
    ...
    if [ -z "$MENTOR_TOKEN" ] || [ -z "$STUDENT_ID" ]; then
        print_fail "Required tokens not available"
        return
    fi
    ...
    if [ -n "$ADMIN_TOKEN" ]; then
        ...
        if echo "$ASSIGN" | grep -q "\"success\":true"; then
            ...
            print_fail "Assignment failed, trying alternate method"
        fi
    fi
    ...
    if echo "$RISK" | grep -q "prediction"; then
        PREDICTION=$(echo $RISK | grep -o '"prediction":"[^"]*' | cut -d'"' -f4)
        print_pass "Risk calculated: $PREDICTION (using fallback)"
    else
